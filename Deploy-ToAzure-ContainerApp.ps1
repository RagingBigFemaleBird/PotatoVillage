...
az acr create --resource-group $ResourceGroupName --name $acrName --sku Basic --admin-enabled true --output none 2>$null
...
az acr build --registry $acrName --image potatovillage-server:latest --file Server/Dockerfile .
...
az containerapp update `
    --name $deployedContainerAppName `
    --resource-group $ResourceGroupName `
    --image "$acrLoginServer/potatovillage-server:latest" `
